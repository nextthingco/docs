...
**Note:** From here on out "host" refers to the Gadget compatible hardware running GadgetOS and "host computer" refers to your development laptop where the "host" is plugged into using USB.
...
To execute commands on all containers on your host machine run the gadget command without any container image name specified.
...
When building and deploying images on your host machine rather than pulling from Docker Hub the syntax above is used. Put the pathname of your Dockerfile and supporting files (the build's context) here in relation to the gadget.yml file.
